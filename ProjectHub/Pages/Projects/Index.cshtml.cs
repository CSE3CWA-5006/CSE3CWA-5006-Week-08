...
public class IndexModel : PageModel
{
    ...
    {
        Q = q;
        var all = _db.Projects.Include(p => p.Tasks).ThenInclude(t => t.Assignees)
                              .OrderBy(p => p.Name).ToList();
        Projects = string.IsNullOrWhiteSpace(q)
            ? all
            : all.Where(p => p.Name.Contains(q, StringComparison.OrdinalIgnoreCase)).ToList();
    }
}
